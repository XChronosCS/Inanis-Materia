...
@export var parent_chain: Node2D = null
@export var ground_y = 0
@onready var psm = PowerStateMachine
signal chain_picked_up
signal chain_released
...
func _on_interact():
	if psm.current_power.alchemical_power == "Earth":
		...
		if get_parent() == player:
			reparent(parent_chain)
			set_collision_mask_value(2, true)
			chain_released.emit()
		else:
			reparent(player)
			set_collision_mask_value(2, false)
			chain_picked_up.emit()
...
func _on_interaction_area_body_exited(body):
	if body.name == "Player":
		...
		if player.state_machine.current_state.name == "Pushing":
			...
			call_deferred("reparent", parent_chain)
